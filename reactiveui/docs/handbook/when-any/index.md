...
This will set an ([ObservableAsPropertyHelper](~/docs/handbook/observable-as-property-helper/index.md) property) field called `_searchTextLength` which you then expose via the `SearchTextLength` property. `ObservableAsPropertyHelper` properties cannot be mutated directly, but are instead calculated via the `WhenAnyValue` selector lambda.

See the [ObservableAsPropertyHelper](~/docs/handbook/observable-as-property-helper/index.md) section for more information on this pattern.
