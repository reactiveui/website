...
[Run the complete NUnit project](https://github.com/reactiveui/refit/tree/main/src/examples/Documentation/TestingFrameworks/NUnit).

This page shows how to test a Refit client with NUnit and `Refit.Testing`. Each test solves one problem,
named in the comment above it.
The same tests exist for [xUnit](xunit.md), [MSTest](mstest.md) and [TUnit](tunit.md). Only the test attributes and assert calls change.
...
```bash
dotnet test examples/Documentation/TestingFrameworks/NUnit/NUnitTests.csproj
```
...
[`Person.cs`](https://github.com/reactiveui/refit/blob/main/src/examples/Documentation/TestingFrameworks/NUnit/Person.cs):
...
[`IPeopleApi.cs`](https://github.com/reactiveui/refit/blob/main/src/examples/Documentation/TestingFrameworks/NUnit/IPeopleApi.cs):
...
[`PersonJsonContext.cs`](https://github.com/reactiveui/refit/blob/main/src/examples/Documentation/TestingFrameworks/NUnit/PersonJsonContext.cs):
...
[`TestSettings.cs`](https://github.com/reactiveui/refit/blob/main/src/examples/Documentation/TestingFrameworks/NUnit/TestSettings.cs):
...
The tests are in [`GettingStartedTests.cs`](https://github.com/reactiveui/refit/blob/main/src/examples/Documentation/TestingFrameworks/NUnit/GettingStartedTests.cs).
...
The tests are in [`ErrorHandlingTests.cs`](https://github.com/reactiveui/refit/blob/main/src/examples/Documentation/TestingFrameworks/NUnit/ErrorHandlingTests.cs).
...
The tests are in [`NetworkConditionsTests.cs`](https://github.com/reactiveui/refit/blob/main/src/examples/Documentation/TestingFrameworks/NUnit/NetworkConditionsTests.cs).
...
public async Task SlowServer_DelaysTheReplyWithoutWaiting()
{
    ...
    Assert.That(pendingPerson.IsCompleted, Is.False); // Only one of the two simulated seconds has passed.
...
public void BrokenNetwork_ThrowsAConnectionError()
{
    ...
    ApiRequestException? error = Assert.ThrowsAsync<ApiRequestException>(() => api.GetPersonAsync(1));
...
The tests are in [`StreamingTests.cs`](https://github.com/reactiveui/refit/blob/main/src/examples/Documentation/TestingFrameworks/NUnit/StreamingTests.cs).
...
public async Task WatchPeople_SeesEachOneAsTheyArrive()
{
    ...
    Assert.That(await people.MoveNextAsync(), Is.True);
    Assert.That(people.Current.Name, Is.EqualTo("Ada"));

    ValueTask<bool> nextPerson = people.MoveNextAsync();
    Assert.That(nextPerson.IsCompleted, Is.False); // Grace has not arrived yet.

    source.Release(new Person(2, "Grace"));
    Assert.That(await nextPerson, Is.True);
    Assert.That(people.Current.Name, Is.EqualTo("Grace"));
...
public async Task UploadPeople_ServerDoesNotWaitForTheWholeUpload()
{
    ...
    Assert.That(producedWhenServerStartedReading, Is.EqualTo(0));
    Assert.That(uploadedJson, Does.Contain("Grace"));
}
```
