...
Two overloads take a value they never send. They read the type off that value, so you do not have to write the
...
The first argument is where to start. The second is how many to send, not where to stop. It also takes an optional
sequencer.
...
captures nothing and allocates no closure. See [mark lambdas `static`](best-practices.md#mark-lambdas-static).
...
operators that do the same job, on the [combination](combination.md), [transformation](transformation.md) and
[error handling](error-handling.md) pages.

Factories that build a stream you push values into yourself, such as `Signal.Serialized`, `Signal.Scheduled`
and `Signal.Delayable`, are covered on the [signals page](signals.md).
