...
worth slowing down on. In the source they had `-1` between them. `Where` dropped the `-1` first, so by the time
...
`KeepWith` passes a state value into the test as its first argument, so you can mark the lambda `static` and it
allocates no closure. See [mark lambdas `static`](best-practices.md#mark-lambdas-static).
...
|---|---|---|
| `OfType<int>()` | `1 3` | Drops it and carries on. |
| `Cast<int>()` | `1`, then an error | Ends the stream with `InvalidCastException`. |

Use `OfType` when mixed types are expected. Use `Cast` when a wrong type means a bug you want to hear
about. `Cast` is on the transformation page.
...
|---|---|---|
| `Where(x => x < 5)` | `1 2 3 4 5` | Tests every value, forever. |
...
`Signal.After` rather than `Signal.Empty` when you want a deadline.
...
|---|---|---|
| `Where` | `Keep` | Values your test rejects. |
