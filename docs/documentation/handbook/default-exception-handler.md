...
You can override this behaviour or hook your debugger or analytics client by registering a custom `IObserver<Exception>` through `RxAppBuilder` — it wires the handler into the read-only `RxState.DefaultExceptionHandler` slot:
